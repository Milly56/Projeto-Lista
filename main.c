...
typedef struct aluno {
	char nome[50];
	char rgm[20];
    struct no *proximo_aluno;
    struct no *proximo_disciplina;
} Aluno;
...
//estrutura da lista
typedef struct no{
	Disciplina dado; // elemento contendo os dados
...
int inserir (Lista * lista, int pos, Aluno dado) {
    ...
    lista->vetor[pos].proximo_disciplina = NULL;
    (lista->n)++;
...
void mostrar(Lista * lista){
	...
	for (i = 0; i <= lista->n; i++) {
		...
		printf("Aluno número %d°", i + 1);
		printf("\nNome: %s \t RGM: %s\n", lista->vetor[i].nome, lista->vetor[i].rgm);
		printf("Disciplina: \n");
	}
}
...
int main(int argc, char *argv[]) {
	...
	if (isCheia(&meusalunos)){
		printf("Situação da Lista: CHEIA\n\n");
	}	
	else{
		printf("Situação da Lista: NÃO CHEIA\n\n");	
	}
	...
	*/
	...
	do{
		puts("\n\n+-------------------------------------------------+");
		puts("|     Digite qual das opções deseja:              |");
		puts("+-------------------------------------------------+");
		puts("|           1 - para remover                      |");
		puts("|           2 - para inserir                      |");
